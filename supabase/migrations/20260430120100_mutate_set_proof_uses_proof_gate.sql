-- P2 (defense in depth): set_proof branches call hh_checklist_proof_upload_gate so signing
-- endpoint and mutate stay aligned if row-level checks are edited in one place later.
...
BEGIN
  ...
  ELSIF p_action = 'set_proof' THEN
    SELECT public.hh_checklist_proof_upload_gate(p_organization_id, p_run_item_id, NULL) INTO v_proof_gate;
    IF COALESCE((v_proof_gate->>'ok')::boolean, false) IS NOT TRUE THEN
      RETURN v_proof_gate;
    END IF;
...
BEGIN
  ...
  ELSIF p_action = 'reopen' THEN
    ...
    END IF;
  ELSIF p_action = 'set_proof' THEN
    SELECT public.hh_checklist_proof_upload_gate(p_organization_id, NULL, p_override_task_id) INTO v_proof_gate;
    IF COALESCE((v_proof_gate->>'ok')::boolean, false) IS NOT TRUE THEN
      RETURN v_proof_gate;
    END IF;
    v_path := nullif(trim(p_payload->>'storage_path'), '');
