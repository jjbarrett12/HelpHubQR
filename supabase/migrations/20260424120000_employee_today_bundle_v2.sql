-- Employee Today bundle v2: run items visible only if unassigned (whole shift) or assigned_employee_id = caller.
-- Adds shift_notes placeholder JSON; bumps source.bundle_version to 2.
...
BEGIN
  IF v_uid IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'source', jsonb_build_object('bundle_version', 2, 'rpc', 'hh_employee_today_bundle')
    );
  END IF;
  ...
  IF v_shift_found THEN
    ...
    IF v_run_found THEN
      ...
      v_progress := jsonb_build_object(
        'completed', (SELECT count(*)::int FROM public.shift_checklist_run_items ri WHERE ri.shift_checklist_run_id = v_run.id AND (ri.assigned_employee_id IS NULL OR ri.assigned_employee_id = v_employee_id) AND ri.completed = true AND COALESCE(ri.suppressed, false) = false),
        'total', (SELECT count(*)::int FROM public.shift_checklist_run_items ri WHERE ri.shift_checklist_run_id = v_run.id AND (ri.assigned_employee_id IS NULL OR ri.assigned_employee_id = v_employee_id) AND COALESCE(ri.suppressed, false) = false),
        'ratio',
          CASE
            WHEN (SELECT count(*) FROM public.shift_checklist_run_items ri WHERE ri.shift_checklist_run_id = v_run.id AND (ri.assigned_employee_id IS NULL OR ri.assigned_employee_id = v_employee_id) AND COALESCE(ri.suppressed, false) = false) = 0 THEN 0::numeric
            ELSE round(
              (SELECT count(*)::numeric FROM public.shift_checklist_run_items ri WHERE ri.shift_checklist_run_id = v_run.id AND (ri.assigned_employee_id IS NULL OR ri.assigned_employee_id = v_employee_id) AND ri.completed = true AND COALESCE(ri.suppressed, false) = false)
              / (SELECT count(*)::numeric FROM public.shift_checklist_run_items ri WHERE ri.shift_checklist_run_id = v_run.id AND (ri.assigned_employee_id IS NULL OR ri.assigned_employee_id = v_employee_id) AND COALESCE(ri.suppressed, false) = false),
              4
            )
          END
      );
...
COMMENT ON FUNCTION public.hh_employee_today_bundle(uuid, text) IS
  'v2: Employee-scoped Today bundle; run items filtered by assigned_employee_id; operational_messages; shift_notes placeholder.';
