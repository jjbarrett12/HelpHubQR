-- Forward / idempotent: DBs that applied shift_notes with `shift_id` + NOT NULL `created_by` get aligned
-- with the canonical schema (`employee_shift_id`, nullable `created_by`, FK ON DELETE SET NULL).
-- Fresh installs from the updated 20260428120000 migration already match; this migration is safe to re-run.
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.shift_notes RENAME COLUMN shift_id TO employee_shift_id;
  END IF;
END;
$$;

DROP INDEX IF EXISTS public.shift_notes_shift_id_created_at_idx;
CREATE INDEX IF NOT EXISTS shift_notes_employee_shift_id_created_at_idx
  ON public.shift_notes (employee_shift_id, created_at);

ALTER TABLE public.shift_notes DROP CONSTRAINT IF EXISTS shift_notes_created_by_fkey;
ALTER TABLE public.shift_notes ALTER COLUMN created_by DROP NOT NULL;
ALTER TABLE public.shift_notes
  ADD CONSTRAINT shift_notes_created_by_fkey
...
COMMENT ON FUNCTION public.hh_employee_today_bundle(uuid, text) IS
  'v4: v3 bundle + shift_notes from public.shift_notes (employee_shift_id; manager-authored, visible_to_employee); bundle_version 4.';
